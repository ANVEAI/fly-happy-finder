...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isHomePage ? 'bg-transparent' : 'bg-card shadow-card'
      }`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 md:h-20">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 group">
            <div className={`p-2 rounded-xl transition-colors ${isHomePage ? 'bg-white/20' : 'bg-primary/10'
              }`}>
              <Plane className={`w-6 h-6 transition-transform group-hover:rotate-12 ${isHomePage ? 'text-white' : 'text-primary'
                }`} />
            </div>
            <span className={`text-xl font-bold ${isHomePage ? 'text-white' : 'text-foreground'
              }`}>
              SkyBook
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Button
              ...
              size="sm"
              className={isHomePage ? 'bg-transparent border-white/30 text-white hover:bg-white/10' : ''}
            >
